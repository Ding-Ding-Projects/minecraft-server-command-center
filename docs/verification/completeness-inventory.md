...
| Guided Java runtime discovery and compatibility review | `src/main/java-runtime-manager.cjs`, `src/main/java-runtime-controller.ts`, `src/shared/desktop-api.ts`, `src/main/index.ts`, `src/preload/index.ts`, `src/renderer/index.html`, `src/renderer/main.ts`, `src/renderer/styles.css` | [Java runtime setup](../reference/java-runtime-setup.md), [architecture](../architecture/desktop-foundation.md) | Source inspected; main-process-owned opaque candidate IDs, fixed direct version probe, Paper target catalog explicitly unavailable, Spigot explicitly unverified, and review-only plan presentation | Not recorded; active ultra-speed lane intentionally ran no test, lint, review, build, package, runtime, or capture |
