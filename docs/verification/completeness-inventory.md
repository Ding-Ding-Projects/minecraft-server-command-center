...
At this documentation revision, the listed shared/main/preload, renderer, Java-runtime guidance, and build/package definitions have source inspection evidence. Planner Handoff v1 is a source-design record whose selected-file, preview, apply/save, and prohibited-route behavior remains unverified. No test, lint, review, accessibility assessment, browser UI interaction, screen capture, release, website publication, source-control publication, runtime, or server-integration result is claimed for this documentation lane. The supported build and package result is reported separately with the exact candidate artifact.
