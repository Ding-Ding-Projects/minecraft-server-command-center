...
| Notification centre | Review bounded browser-local non-blocking notices, their active or dismissed state, and scoped bulk dismissal. | Send notices remotely, claim server delivery, or delete review records. |
| Release status | Show one source-embedded, verified version-pinned release record and direct installer handoff. | Invent a release, fetch a release feed, start or observe a download, or claim deployment or installation. |
