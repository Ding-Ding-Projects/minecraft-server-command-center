...
export const NOTIFICATION_STORAGE_KEY = "minecraft-server-command-center.site.notifications.v1";
...
export function parseNotificationCenter(value: unknown): ParseResult {
  if (!isRecord(value) || !hasExactKeys(value, NOTIFICATION_STATE_KEYS)) {
    return { ok: false, reason: "The notification record did not match schema v1." };
  }
  if (value.schemaVersion !== NOTIFICATION_SCHEMA_VERSION || !Array.isArray(value.records)) {
    return { ok: false, reason: "The notification record used an unsupported schema or shape." };
  }
  if (value.records.length > MAX_NOTIFICATION_RECORDS || !value.records.every(isNotificationRecord)) {
    return { ok: false, reason: "The notification record exceeded its bounded record or field limits." };
  }
  const ids = value.records.map((record) => record.id);
  if (new Set(ids).size !== ids.length) {
    return { ok: false, reason: "The notification record contained duplicate identifiers." };
  }
  return {
    ok: true,
    value: {
...
export function serializeNotificationCenter(state: NotificationCenterState): string {
  ...
  if (new TextEncoder().encode(serialized).byteLength > MAX_NOTIFICATION_STORAGE_BYTES) {
    throw new Error("The local notification record exceeded its storage bound.");
  }
  return serialized;
}
...
export function createNotificationRecord(input: NotificationInput, now = new Date()): NotificationRecord {
  ...
  if (!isNotificationRecord(record)) throw new Error("Notification content exceeded its bounded local contract.");
  return record;
}
