...
const DESKTOP_COPY: Record<DesktopPresentationKey, DesktopCopyDefinition> = {
  ...
  "settings.personalVocabulary.retry": {
    english: ["Retry cache cleanup", "Retry cache cleanup", "Retry local vocabulary cache cleanup", "Retry local vocabulary cache cleanup after the previous removal failed", "Retry cache cleanup; the malformed file gets one more polite exit attempt"],
    cantonese: ["重試清理快取", "重試清理快取", "重試清理本機詞彙快取", "上次移除失敗後重試清理本機詞彙快取", "重試清理快取；格式錯誤檔案再試一次有禮貌退場"],
  },
  "settings.personalVocabulary.notice.loaded": {
  ...
  "settings.personalVocabulary.command.retry": {
    english: ["Retry vocabulary cache cleanup", "Retry vocabulary cache cleanup", "Retry local vocabulary cache cleanup", "Retry cleanup after the malformed cache could not be removed", "Retry vocabulary cache cleanup; the stuck cache gets another stage cue"],
    cantonese: ["重試詞彙快取清理", "重試詞彙快取清理", "重試本機詞彙快取清理", "格式錯誤快取未能移除後重試清理", "重試詞彙快取清理；卡住嘅快取再收一次場"],
  },
  "settings.personalVocabulary.command.retryDescription": {
    english: ["Open Settings and focus the retry control after malformed-cache cleanup fails.", "Open Settings and focus the retry control after malformed-cache cleanup fails.", "Open Settings and focus the local retry control; no file path is exposed.", "Open Settings and retry removal of the malformed local cache without changing protected data.", "Open Settings and focus retry; the cache gets a second chance to leave without drama."],
    cantonese: ["開啟設定並聚焦格式錯誤快取清理重試掣。", "開啟設定並聚焦格式錯誤快取清理重試掣。", "開啟設定並聚焦本機重試掣；唔會暴露檔案路徑。", "開啟設定並重試移除格式錯誤本機快取，唔會改動受保護資料。", "開啟設定並聚焦重試；快取有第二次機會安靜退場。"],
  },
  "settings.personalVocabulary.picker.title": {
